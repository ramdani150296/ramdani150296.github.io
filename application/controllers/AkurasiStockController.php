...
class AkurasiStockController extends CI_Controller {
        ...
        if(!isset($_FILES['fileExcel']['tmp_name']) || $_FILES['fileExcel']['tmp_name'] == ""){
            ...
            if(!$readerExcelFile->canRead($temporaryFile)){
                ...
                if($totalRows > $maxUploadedRow){
                    ...
                    $query = "INSERT INTO ".$this->AkurasiStockModel->getTable()."".$columnsName." VALUES ".$columnsValues.";";
    ...
    public function getAllData(){
        ...
        foreach ($list as $cts) {
			...
			$row[] = $cts->storage_type;
			$row[] = $cts->storage_bin;
			$row[] = $cts->total_stock;
			$row[] = $cts->unposted;
			$row[] = $cts->stock_onhand;
			$row[] = $cts->stock_good;
			$row[] = $cts->stock_bad;
			$row[] = $cts->diff_qty;
			$row[] = $cts->bin_accurasi;
			$row[] = $cts->std_price;
			$row[] = $cts->onhand_val;
			$row[] = $cts->physic_val;
			$row[] = $cts->div_val;
        ...
        $output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->AkurasiStockModel->countAllData(),
			"recordsFiltered" => $this->AkurasiStockModel->countFilteredData(),
			"data" => $data,
		);
